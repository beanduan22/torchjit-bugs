"""
Bug 9: TorchJIT numerical inconsistency — torch 2.11.0+cu128
Max diff: 6.6
Ops: ['Constant', 'Input', 'PTMatMul', 'Tan', 'Sigmoid', 'PReLU', 'Round', 'Linear', 'Floor', 'Flatten', 'CastF64', 'Tan']
...
BUG_DIR = Path("/home/binduan/myspace/compilerTesting/nnsmith_results/torchjit_2.11/bug-Symptom.INCONSISTENCY-Stage.VERIFICATION-9")
...
trace_inputs = [
    torch.tensor([3.343278408050537], dtype=torch.float32),
]
# eval_inputs: the oracle inputs that expose the divergence
eval_inputs = [
    torch.tensor([4.598626613616943], dtype=torch.float32),
]
...
print(f"Bug 9: torch {torch.__version__}")
